...
# ============================================
# HTML TEMPLATES & CSS (With Dark Mode Variables)
# ============================================
BASE_STYLES = """
<style>
    ...
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    @media print {
        ...
        .sidebar, .top-actions, .date-range-picker, .tabs, .month-selector { display: none !important; }
...
def ACTION_BAR_HTML():
    ...
    <div class="top-actions">
        <div class="search-box">
            ...
            <input type="text" id="global-search" class="search-input" placeholder="Search Order ID..." onkeyup="if(event.key==='Enter') searchOrder(this.value)">
...
function renderProvider(provider) {
    ...
    const subHdr = days.map((_,i) => `<th${flightDays.includes(i)?' style="background:var(--hover-bg)"':''}><div class="sub-header"><span>O</span><span>B</span><span>W</span></div></th>`).join('');
...
<div id="content"><div class="empty-state"><div class="empty-state-icon">📅</div><h3>Select a date range</h3></div></div>
</main>
''' + SIDEBAR_SCRIPT + SHARED_JS + '''
<script>
async function loadData() {
...
<div class="card-header" style="cursor:pointer" onclick="document.getElementById('bdy-${idx}').style.display = document.getElementById('bdy-${idx}').style.display === 'none' ? 'block' : 'none'">
<div class="provider-info"><div class="provider-color" style="background:${provider.color}"></div><span class="provider-name">${provider.name}</span></div>
<div class="card-stats">
<div class="stat-item"><div class="stat-value">${provider.orders}</div><div class="stat-label">Orders</div></div>
<div class="stat-item"><div class="stat-value">${provider.boxes}</div><div class="stat-label">Boxes</div></div>
</div></div>
<div id="bdy-${idx}" style="display:${idx===0?'block':'none'}"><table class="data-table"><thead><tr><th>Region</th><th style="text-align:right">Orders</th><th style="text-align:right">Boxes</th><th style="text-align:right">Weight</th></tr></thead><tbody>`;
            provider.regions.forEach(rg => {
                html += `<tr><td class="region-col">${rg.name}</td>
                    <td style="text-align:right"><a href="/orders?provider=${encodeURIComponent(provider.name)}&start=${fmtLocal(dpStart)}&end=${fmtLocal(dpEnd)}&region=${encodeURIComponent(rg.name)}" class="orders-link">${rg.orders}</a></td>
                    <td style="text-align:right"><a href="/orders?provider=${encodeURIComponent(provider.name)}&start=${fmtLocal(dpStart)}&end=${fmtLocal(dpEnd)}&region=${encodeURIComponent(rg.name)}" class="boxes-link">${rg.boxes}</a></td>
                    <td style="text-align:right"><a href="/orders?provider=${encodeURIComponent(provider.name)}&start=${fmtLocal(dpStart)}&end=${fmtLocal(dpEnd)}&region=${encodeURIComponent(rg.name)}" class="weight-link">${formatWeight(rg.weight)}</a></td></tr>`;
            });
...
def api_search():
    ...
    for provider in PROVIDERS:
        ...
        for row_idx, row in enumerate(rows):
            ...
            if order_col < len(row) and query in row[order_col].strip().lower():
                try:
                    date_val = row[provider['date_col']].strip() if provider['date_col']<len(row) else ''
                    ...
                    results.append({
                        'provider': provider['name'], 'order_id': row[order_col], 'date': date_str, 'region': region, 'weight': weight, 'color': provider['color']
                    })
...
def api_flight_load():
    ...
    flights = [{'name': 'Tuesday Flight (Mon + Tue)', 'days': ['Mon', 'Tue']}, {'name': 'Thursday Flight (Wed + Thu)', 'days': ['Wed', 'Thu']}, {'name': 'Saturday Flight (Fri + Sat)', 'days': ['Fri', 'Sat']}]
    flight_data = []
        ...
        for provider in providers_data:
            ...
            for region_data in provider['regions'].values():
                for day in flight['days']:
                    ...
                    pf['orders'] += dd.get('orders', 0); pf['boxes'] += dd.get('boxes', 0); pf['weight'] += dd.get('weight', 0)
            fi['total_orders'] += pf['orders']; fi['total_boxes'] += pf['boxes']; fi['total_weight'] += pf['weight']
...
def api_daily_region_summary():
    start_date, end_date = parse_date_range(request)
    result = {'totals': {'orders': 0, 'boxes': 0, 'weight': 0.0, 'under20': 0, 'over20': 0}, 'providers': []}
    for provider in PROVIDERS:
        ...
        for row_idx, row in enumerate(rows):
            ...
            try:
                ...
                if region not in pd['regions']: pd['regions'][region] = {'name': region, 'orders': 0, 'boxes': 0, 'weight': 0.0, 'under20': 0, 'over20': 0}
                pd['regions'][region]['orders'] += 1; pd['regions'][region]['boxes'] += boxes; pd['regions'][region]['weight'] += weight
        ...
        result['totals']['orders'] += pd['orders']; result['totals']['boxes'] += pd['boxes']; result['totals']['weight'] += pd['weight']
        result['totals']['under20'] += pd['under20']; result['totals']['over20'] += pd['over20']
...
def api_analytics_data():
    start_date, end_date = parse_date_range(request)
    result = {'totals': {'orders': 0, 'boxes': 0, 'weight': 0.0, 'under20': 0, 'over20': 0}, 'trend': {'labels': [], 'orders': [], 'boxes': []}, 'providers': [], 'regions': []}
    ...
    for provider in PROVIDERS:
        ...
        for row_idx, row in enumerate(rows):
            ...
            try:
                ...
                if days_diff <= 1: date_key = parsed_date.strftime('%H:00')
                elif days_diff <= 31: date_key = parsed_date.strftime('%b %d')
                else: date_key = parsed_date.strftime('%b %Y')
                trend_data[date_key]['orders'] += 1; trend_data[date_key]['boxes'] += boxes
            except: continue
    if days_diff <= 1: labels = [f'{h:02d}:00' for h in range(24)]
    elif days_diff <= 31: labels = [(start_date + timedelta(days=i)).strftime('%b %d') for i in range(days_diff)]
    else:
        seen = []; d = start_date
        while d <= end_date:
            lbl = d.strftime('%b %Y')
            if lbl not in seen: seen.append(lbl)
            d = d + timedelta(days=32); d = d.replace(day=1)
        labels = seen
    result['trend']['labels'] = labels
    for lbl in labels:
        result['trend']['orders'].append(trend_data[lbl]['orders']); result['trend']['boxes'].append(trend_data[lbl]['boxes'])
...
def api_monthly():
    ...
    weeks_data = []; current = start_date; week_num = 1
...
def api_calendar():
    ...
    day_names = ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun']
    days_data = {day: {'day': day, 'orders': 0, 'boxes': 0, 'weight': 0.0, 'under20': 0, 'over20': 0} for day in range(1, num_days + 1)}
    ...
    return jsonify({'year': year, 'month': month, 'first_weekday': first_weekday, 'totals': {'orders': sum(d['orders'] for d in days_data.values()), 'boxes': sum(d['boxes'] for d in days_data.values()), 'weight': sum(d['weight'] for d in days_data.values()), 'under20': sum(d['under20'] for d in days_data.values()), 'over20': sum(d['over20'] for d in days_data.values())}, 'max_boxes': max((d['boxes'] for d in days_data.values()), default=1), 'days': list(days_data.values())})
...
def api_whatsapp():
    ...
    for provider in PROVIDERS:
        data = process_provider_data(provider, start_date, end_date)
        if data:
            providers_data.append(data); total_orders += data['total_orders']; total_boxes += data['total_boxes']; total_weight += data['total_weight']
    providers_data.sort(key=lambda x: x['total_boxes'], reverse=True)
    report = f"📊 *3PL Report*\n📅 {start_date.strftime('%b %d')} - {end_date.strftime('%b %d, %Y')}\n\n━━━━━━━━━━━━━━━━━━━━\n\n🏆 *PROVIDER RANKING*\n\n"
    medals = ['🥇','🥈','🥉','4️⃣','5️⃣','6️⃣']
    for i, p in enumerate(providers_data): report += f"{medals[i] if i<6 else '🔸'} *{p['short']}*\n   📦 {p['total_boxes']:,} boxes | ⚖️ {p['total_weight']:,.1f} kg\n\n"
    ...
    for provider in PROVIDERS:
        ...
        for row_idx, row in enumerate(rows):
            ...
            try:
                ...
                if region not in result['regions']: result['regions'][region] = {'name': region, 'orders': 0, 'boxes': 0, 'weight': 0.0, 'under20': 0, 'over20': 0}
...
<head>
    ...
    <style>
        body { padding: 20px; }
        .back-btn { display: inline-block; margin-bottom: 20px; padding: 8px 16px; background: var(--brand-color); color: #ffffff; text-decoration: none; border-radius: 6px; font-weight: 600; }
