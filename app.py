...
# ==============================================================================
# 📦 BUNDLING INTELLIGENCE HUB - FINAL STABLE (PERFECT MAPPING)
# ==============================================================================
...
def fetch_bundling_standalone_data():
    now = time.time()
    if 'bundle_data' in CACHE and (now - CACHE['bundle_time']) < CACHE_DURATION:
        return CACHE['bundle_data']
    ...
    BUNDLING_SOURCES = {
        ...
        "ECL Zone": (
            ...
            # A=0, B=1, E=4, I=8, N=13, O=14, P=15, Q=16, U=20, AC=28
    ...
    try:
        ...
        CACHE['bundle_data'] = res
        CACHE['bundle_time'] = now
    except Exception as e:
        pass
    return res
...
@app.after_request
def add_global_floating_btns(response):
    if request.path == '/' and response.content_type and 'text/html' in response.content_type:
        user_val = session.get('username') or session.get('user') or session.get('role')
        if user_val and str(user_val).lower() == 'admin':
            html = response.get_data(as_text=True)
            # Floating buttons siraf aur siraf Main Dashboard ke kone mein dikhenge
            btn = '''
