...
def fetch_bundling_exact_data():
    MAPS = {
        ...
        "ECL Zone": ("https://docs.google.com/spreadsheets/d/e/2PACX-1vSCiZ1MdPMyVAzBqmBmp3Ch8sfefOp_kfPk2RSfMv3bxRD_qccuwaoM7WTVsieKJbA3y3DF41tUxb3T/pub?gid=928309568&single=true&output=csv", {"o": 0, "d": 1, "b": 3, "oli": 11, "v": 13, "title": 14, "ic": 15, "c": 16, "cn": 20, "t": 28}),
    ...
    for name, (url, col) in MAPS.items():
        try:
            ...
            with urllib.request.urlopen(req, timeout=20) as r:
                ...
                for row in data[1:]:
                    p = row + ['']*60
                    oid = str(p[col['o']]).strip()
                    if not re.search(r'\d', oid) or oid.lower() in ['order','orderid']: continue
                    def g(c, is_box=False): 
                        v=str(p[c]).strip()
                        if is_box: return v if v else ""
                        return v if v and v.lower() not in ['n/a','nan','-'] else "N/A"
                    processed.append({'order':oid,'date':g(col['d']),'date_std':std_date(g(col['d'])),'boxes':g(col['b'], True),'oli':g(col['oli']),'v':g(col['v']),'title':g(col['title']),'ic':g(col['ic']),'c':g(col['c']),'cn':g(col['cn']),'t':g(col['t'])})
                res[name] = processed
        except: continue
    return res
...
@app.route('/api/nexus/ops_commander', methods=['GET'])
def api_nexus_ops():
    sd, kd = fetch_nexus_data(); blame = []; mt = "Hi Kerry Team,\nMissing TIDs for Handed Over orders:\n\n"; count = 1
    for src, rows in sd.items():
        for r in rows:
            try: d_obj = datetime.strptime(r['date'].split(' ')[0], '%Y-%m-%d')
            except: continue
            o = r['order']; ks = str(kd.get(o.lower(), "PENDING")).upper().replace(" ", "").replace("_", "")
            ht = len(clean_tids(r['tid'])) > 0; days = (datetime.now() - d_obj).days
            if "HANDEDOVER" in ks and not ht and days > 1:
                blame.append({"order": o.upper(), "source": src, "issue": "Missing TID", "aging": f"{days} Days", "blame": "Kerry"})
                mt += f"{count}. Order: {o.upper()} | Date: {r['date']} | Source: {src}\n"; count += 1
            elif ks in ["QCPENDING", "CREATED", "ACCEPTED"] and days > 2:
                blame.append({"order": o.upper(), "source": src, "issue": "Stuck in QC", "aging": f"{days} Days", "blame": f"{src} Ops"})
    if count == 1: mt = "All good! No missing TIDs."
    return jsonify({"blame_radar": sorted(blame, key=lambda x: int(x['aging'].split()[0]), reverse=True), "missing_text": mt})
    ...
    for src, rows in all_sheets.items():
        ...
        for r in rows:
            if r['boxes'] != "": 
                ...
                t_list = clean_tids(r['tid'])
                cb = {"orders": [{"order_id": r['order'].upper(), "order_line_id": r['order_line_id'], "title": r['title'], "item_count": r['item_count'], "country": r['country']}], "date": r['date'], "date_std": r['date_std'], "customer": r['customer'], "vendor": r['vendor'], "country": r['country'], "source": src, "boxes_val": r['boxes'], "tid": ", ".join(t_list) if t_list else "Pending", "total_items": 0}
            else:
                if cb:
                    cb['orders'].append({"order_id": r['order'].upper(), "order_line_id": r['order_line_id'], "title": r['title'], "item_count": r['item_count'], "country": r['country']})
                    if r['tid'] != "N/A" and r['tid'] != "":
                        if cb['tid'] == "Pending": cb['tid'] = r['tid']
        if cb and len(cb['orders']) > 1:
            bundles.append(cb); t_b += 1; t_o += len(cb['orders']); ss[src]["o"] += len(cb['orders'])
            try: ss[src]["b"] += int(float(cb['boxes_val']))
            except: pass
...
@login_required
def dashboard():
    return render_template_string(f'<!DOCTYPE html><html><head><title>Main Dashboard</title>{FAVICON}{BASE_STYLES}</head><body><h1>📦 3PL Operations Dashboard</h1><div class="card"><p>Welcome to the main WMS Dashboard. Check bottom right for App Hub buttons.</p><a href="/nexus" style="display:inline-block;margin-right:10px;padding:10px 20px;background:#000;color:#fff;text-decoration:none;border-radius:8px;font-weight:bold;">Launch TID Hub</a><a href="/bundling" style="display:inline-block;padding:10px 20px;background:#10B981;color:#fff;text-decoration:none;border-radius:8px;font-weight:bold;">Launch Bundling</a></div></body></html>')
...
def bundling_view():
    if session.get('role') != 'admin': return "Admin Only", 403
    return render_template_string('''<!DOCTYPE html><html data-theme="dark"><head><title>📦 Bundling Pro</title><style>body{font-family:'Inter',sans-serif;background:#000;color:#fff;padding:40px;margin:0;padding-bottom:100px;}.hdr{display:flex;justify-content:space-between;margin-bottom:30px;border-bottom:1px solid #222;padding-bottom:20px;}.kpis{display:grid;grid-template-columns:repeat(3,1fr);gap:20px;margin-bottom:30px;}.kpi{background:#0A0A0A;border:1px solid #1A1A1A;padding:20px;border-radius:12px;border-left:4px solid #10B981;}.val{font-size:32px;font-weight:900;}.flt{display:flex;gap:15px;background:#0A0A0A;padding:15px;border-radius:12px;border:1px solid #1A1A1A;margin-bottom:30px;}select,input{background:#111;color:#fff;border:1px solid #333;padding:10px;border-radius:6px;}table{width:100%;border-collapse:collapse;background:#0A0A0A;}th,td{padding:15px;border-bottom:1px solid #1A1A1A;text-align:left;}th{color:#888;font-size:12px;text-transform:uppercase;background:#050505;}.bbox{background:#050505;padding:10px;border-radius:8px;border:1px solid #1A1A1A;}.float{position:fixed;bottom:30px;right:30px;display:flex;gap:10px;}.btn{padding:10px 20px;border-radius:50px;font-weight:bold;cursor:pointer;border:none;}</style></head><body><div class="hdr"><div><h1 style="margin:0;color:#10B981;">📦 Bundling AI</h1><p style="color:#888;margin:5px 0 0 0;">Advanced Order Consolidation</p></div></div><div class="kpis"><div class="kpi"><div class="val" id="kb">0</div><small>Total Bundles</small></div><div class="kpi"><div class="val" id="ko">0</div><small>Orders Merged</small></div><div class="kpi" style="border-color:#F59E0B;"><div class="val" id="ks" style="color:#F59E0B">0</div><small>Shipments Saved</small></div></div><div class="flt"><select id="fs"><option value="ALL">All Sources</option><option value="ECL QC Center">ECL QC</option><option value="ECL Zone">ECL Zone</option><option value="GE Zone">GE Zone</option></select><input type="date" id="fd1"><input type="date" id="fd2"><button class="btn" style="background:#fff;color:#000;" onclick="loadB()">Apply Filters</button></div><div id="ld" style="text-align:center;color:#888;display:none;">Scanning Merged Rows...</div><table id="tbl"><thead><tr><th>Source / Date</th><th>Customer Info</th><th>Box & Tracking</th><th>Items Breakdown</th></tr></thead><tbody id="tb"></tbody></table><div class="float"><a href="/" class="btn" style="background:#222;color:#fff;text-decoration:none;">🏠 Dash</a><button onclick="loadB()" class="btn" style="background:#10B981;color:#fff;">🔄 Refresh</button></div><script>async function loadB(){document.getElementById('tbl').style.display='none';document.getElementById('ld').style.display='block';try{const r=await fetch('/api/nexus/bundling_data');const d=await r.json();document.getElementById('kb').innerText=d.kpi.total_bundles;document.getElementById('ko').innerText=d.kpi.total_orders_bundled;document.getElementById('ks').innerText=d.kpi.saved_shipments;const sf=document.getElementById('fs').value,s1=document.getElementById('fd1').value,s2=document.getElementById('fd2').value;let h='';d.bundles.forEach(b=>{if(sf!=='ALL'&&b.source!==sf)return;if(s1&&b.date_std<s1)return;if(s2&&b.date_std>s2)return;let i=b.orders.map(o=>`<div style="padding:5px 0;border-bottom:1px dashed #222;font-size:13px;display:flex;justify-content:space-between;"><span><b style="color:#10b981">${o.order_id}</b><br><span style="color:#888;font-size:11px;">${o.title}</span></span><b>Qty: ${o.item_count}</b></div>`).join('');h+=`<tr><td><b>${b.date}</b><br><small style="color:#888">${b.source}</small></td><td><b>${b.customer}</b><br><small style="color:#666">${b.vendor} (${b.country})</small></td><td><div style="background:#111;padding:8px;border-radius:6px;"><small style="color:#888">TID:</small> <span style="font-family:monospace">${b.tid}</span><br><small style="color:#888">BOX:</small> <b style="color:#10b981">${b.boxes_val}</b></div><b>Items: ${b.total_items}</b></td><td><div class="bbox">${i}</div></td></tr>`;});document.getElementById('tb').innerHTML=h||'<tr><td colspan="4" style="text-align:center;padding:40px;">No bundles found.</td></tr>';document.getElementById('tbl').style.display='table';document.getElementById('ld').style.display='none';}catch(e){document.getElementById('ld').innerHTML='<span style="color:red">Error loading data</span>';}} window.onload=loadB;</script></body></html>''')
