...
# ==============================================================================
# 📦 BUNDLING INTELLIGENCE HUB - STANDALONE COMPLETE CODE
# ==============================================================================
import urllib.request
import csv
import re
...
# ==============================================================================
# BUNDLING TOOL - FLOATING BUTTON (COMPLETELY STANDALONE)
# ==============================================================================
@app.after_request
def add_bundling_floating_btn(response):
    # Sirf HTML pages par inject karo
    if response.content_type and 'text/html' in response.content_type:
        # Sirf home page (/) par inject karo, /bundling par nahi
        if request.path == '/':
            user_val = session.get('username') or session.get('user') or session.get('role')
            if user_val and str(user_val).lower() == 'admin':
                html = response.get_data(as_text=True)
                btn = '<a href="/bundling" style="position:fixed; bottom:100px; right:30px; background:linear-gradient(135deg, #10B981, #059669); color:#fff; border:none; padding:14px 28px; border-radius:50px; text-decoration:none; font-weight:700; z-index:9999; font-family:\'Inter\',sans-serif; box-shadow:0 10px 25px -5px rgba(0,0,0,0.5); transition:0.3s;" onmouseover="this.style.transform=\'translateY(-3px)\'" onmouseout="this.style.transform=\'translateY(0)\'">📦 Bundling AI</a>'
                if '</body>' in html:
                    response.set_data(html.replace('</body>', btn + '</body>'))
    return response
# ==============================================================================
# END OF BUNDLING TOOL CODE
# ==============================================================================
