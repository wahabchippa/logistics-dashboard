...
.ag-breach-row td{background:rgba(239,68,68,.03)!important;}
.ag-warn-row td{background:rgba(245,158,11,.02)!important;}
...
.j-shd{font-size:10px;font-weight:800;text-transform:uppercase;letter-spacing:1.2px;color:var(--brand-color);margin:16px 0 10px;padding-bottom:7px;border-bottom:1px solid var(--border-color);}
...
<div class="date-range-picker" style="margin-bottom:20px">
  <div style="display:flex;gap:12px;align-items:flex-end;flex-wrap:wrap">
    <div>
      ...
      <div class="qbtns-row" style="margin:0;gap:6px">
        <button class="qbtn active" onclick="agFilter(this,'all')">All</button>
        <button class="qbtn" onclick="agFilter(this,'breach')" style="color:#ef4444">🔴 Breach</button>
        <button class="qbtn" onclick="agFilter(this,'warn')" style="color:#f59e0b">🟡 Warning</button>
        <button class="qbtn" onclick="agFilter(this,'ok')" style="color:#10b981">🟢 On Time</button>
      </div>
    </div>
    <div>
      <div style="font-size:10px;color:var(--text-muted);margin-bottom:5px;font-weight:700;text-transform:uppercase">Provider</div>
      <select id="agSrc" class="range-input" onchange="agApply()" style="padding:6px 12px">
    ...
    <div style="flex:2;min-width:300px">
      ...
      <input type="text" id="agQ" class="range-input" style="width:100%;font-size:14px;padding:8px 16px;border-radius:30px;" placeholder="Type Order ID to search..." oninput="agApply()">
    </div>
    <button class="apply-btn" onclick="loadAging()" style="padding:8px 20px">🔄 Refresh</button>
  </div>
</div>
...
''' + SIDEBAR_SCRIPT + SHARED_JS + '''
<script>
let _agAll=[], _agSla='all', _agFilt=[];
const AG_PAGE=100; let _agPage=0;
...
function agApply(){
  ...
  const avg=ages.length?(ages.reduce((a,b)=>a+b,0)/ages.length).toFixed(1)+'d':'—';
  document.getElementById('agKpi').innerHTML=`
    ...
    <div class="stat-card"><div class="stat-icon" style="background:rgba(79,70,229,.1)">⏱️</div>
      <div class="stat-content"><div class="stat-value">${avg}</div><div class="stat-label">Avg QC→Handover</div></div></div>`;
  _agPage=0;
...
function agRenderPage(){
  const start=_agPage*AG_PAGE;
  const slice=_agFilt.slice(start,start+AG_PAGE);
...
function colFor(v){
  if(!v||v==='N/A') return 'var(--text-muted)';
  const n=parseFloat(v); if(isNaN(n)) return 'var(--text-muted)';
...
async function openJ(oid){
  ...
  try{
    ...
    let sh='';
    if(steps.length){
      sh='<div class="j-shd">⏱️ Step Durations</div><div class="stp-grid">';
    ...
    const ti=(lb,v,type)=>{
      ...
      const valCls=v?'':(type==='c'?'canc-v':'pend');
    ...
    document.getElementById('jBody').innerHTML=`
      ...
      ${cb}${slaBadge}
      <div class="j-shd">⭐ Key Metrics</div>
      <div class="km-grid">
      ...
      ${sh}
      <div class="j-shd">🗺️ Full Timeline</div>
      <div class="tl-wrap">
    ...
    for provider in PROVIDERS:
        ...
        for row in sheet_rows[start:]:
            ...
            if not oid or oid in seen:
                continue
            if oid.lower() in ['','nan','order','orderid','fleek id','fleek_id','#n/a','date','cartons']:
                continue
            ...
            if region.upper() in {'N/A','#N/A','COUNTRY','REGION','DESTINATION','ZONE','','ORDER','FLEEK ID','DATE','CARTONS'}:
                region = ''
            j   = jm.get(oid)
